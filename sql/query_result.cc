...
bool Query_result_mq::send_result_set_metadata(THD *thd, const mem_root_deque<Item*> &list MY_ATTRIBUTE((unused)),
...
bool Query_result_mq::send_data(THD *thd, const mem_root_deque<Item *> &items MY_ATTRIBUTE((unused))) {
  ...
  assert((uint)null_num == 2 * (send_fields_size - MQ_FIELDS_DATA_HEADER_LENGTH));
