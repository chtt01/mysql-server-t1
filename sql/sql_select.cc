...
bool JOIN::make_pq_tables_info() {
  ...
  TABLE *table = nullptr;
  mem_root_deque<Item *> table_item_list(thd->mem_root);
  ...
  query_result = down_cast<Query_result_mq*>(query_expression()->query_result());
  ...
  */
  ...
  for (Item *tmp_item : *curr_fields) {
    // check const_item
    if (tmp_item->skip_create_tmp_table) {
      assert(tmp_item->const_item());
      continue;
    }
  ...
  if (table_item_list.size() != table->s->fields
  ...
  */
  tmp_fields[REF_SLICE_PQ_TMP] = table_item_list;
  DBUG_RETURN(false);
...
bool JOIN::make_leader_tables_info() {
  ...
  mem_root_deque<Item *> tmplist(*curr_fields);
  ...
  for (Item *tmp_item : *curr_fields) {
    if (tmp_item->type() == Item::FIELD_ITEM || tmp_item->type() == Item::DEFAULT_VALUE_ITEM) {
      ...
      if (tmp_item->const_item()) {
        uint32 length = (down_cast<Item_field *>(tmp_item)->base_item_field())->field->data_length();
        memmove((down_cast<Item_field *>(tmp_item)->result_field)->ptr,
               (down_cast<Item_field *>(tmp_item)->base_item_field())->field->ptr,
               length);
      }
    }
    i++;
  ...
  */
  ...
  if (pq_build_sum_funcs(thd, query_block, ref_items[REF_SLICE_PQ_TMP],
      tmp_fields[REF_SLICE_PQ_TMP], CountVisibleFields(tmp_fields[REF_SLICE_PQ_TMP]),
      (nesting_map)1 << (unsigned int) query_block->nest_level)) {
    DBUG_RETURN(true);
  }
  ...
  if (!group_list.empty() || tmp_table_param->sum_func_count) {
     if (make_sum_func_list(*curr_fields, true, true)) {
       DBUG_RETURN(true);
     }
  }
  ...
  if (need_tmp_before_win) {
    ...
    */
    ...
    if (create_intermediate_table(&qep_tab[curr_tmp_table], *curr_fields,
                                  tmp_group, !group_list.empty() && simple_group))
      DBUG_RETURN(true);
    ...
    */
    ...
    if (streaming_aggregation || qep_tab[curr_tmp_table].table()->group ||
      ...
      if (change_to_use_tmp_fields(curr_fields, thd,
                                   ref_items[REF_SLICE_TMP1],
                                   &tmp_fields[REF_SLICE_TMP1],
                                   query_block->m_added_non_hidden_fields))
        DBUG_RETURN(true);
    } else {
      if (change_to_use_tmp_fields_except_sums(curr_fields, thd, query_block,
                                    ref_items[REF_SLICE_TMP1],
                                    &tmp_fields[REF_SLICE_TMP1],
                                    query_block->m_added_non_hidden_fields))
        DBUG_RETURN(true);
    ...
    if ((!group_list.empty() &&
      ...
      */
      ...
      if (!group_list.empty()) {
        ...
        {
          if (add_sorting_to_table(curr_tmp_table - 1, &group_list, false, true))
            DBUG_RETURN(true);
...
// ======================================
  ...
  */
  ...
  if (qep_tab && (!group_list.empty() || (!order.empty() && !m_windowing_steps /* [1] */))) {
    /*
    ...
    */
    ...
    if (qep_tab &&
        ...
        (group_list.empty() ? ORDERED_INDEX_ORDER_BY : ORDERED_INDEX_GROUP_BY) &&
      ...
      if (add_sorting_to_table(curr_tmp_table, &order_arg, pq_stable_sort, false))
        return true;
      /*
        ...
        query_expression()->select_limit_cnt == 1 (we only need one row in the result set)
      */
  ...
  for (Item *tmp_item : *fields) {
    if ((tmp_item->type() == Item::FIELD_ITEM || tmp_item->type() == Item::DEFAULT_VALUE_ITEM) &&
            saved_join_fields[i] && saved_join_fields[i]->orig_table_name) {
      (down_cast<Item_field *>(tmp_item))->field->orig_table_name = saved_join_fields[i]->orig_table_name;
    }
    i++;
  }
    ...
    */
    ...
    if (qep_tab &&
      ...
      if (add_sorting_to_table(curr_tmp_table, &order_arg,
                               /*force_stable_sort=*/pq_stable_sort,
