  ...
  for (;;) {
    ...
    */
    ...
    int workers_in_need = (int)threadpool_toobusy - 
      ...
      - (listener_picks_event? 1 : 0);

    /* There are no remaining threads and the thread group is stalled */
    if (workers_in_need <= 0 && !listener_picks_event &&
    ...
    int really_need = listener_picks_event? (cnt - 1) : cnt;
    workers_in_need = workers_in_need > really_need ? really_need : workers_in_need;
    ...
    for (int i = 0; i < workers_in_need; i++) {
      ...
      if (wake_thread(thread_group, false)) {
        /*
          Wake failed, hence groups has no idle threads.
...
static void handle_event(connection_t *connection) {
  ...
  while (1) {
    ...
    if (connection->thd->is_admin_connection()) {
      connection->thd->skip_wait_timeout = false;
    } else {
      connection->thd->skip_wait_timeout = true;
      break;
    }
  }
...
static void *admin_port_worker_main(void *param) {
  ...
  DBUG_ASSERT(connection != nullptr);
  DBUG_ASSERT(connection->thread_group != nullptr);
  thread_group_t *group = connection->thread_group;
...
static void *worker_main(void *param) {
  ...
  DBUG_ASSERT(thread_group != nullptr);
