...
TABLE_LIST *get_next_table(TABLE_LIST *start_table,
...
TABLE_LIST *get_table_by_index(TABLE_LIST *start_table,
...
int get_table_index(TABLE_LIST *start_table, table_list_type_enum list_type,
                    TABLE_LIST *tl) {
  if (start_table == nullptr) {
    return -1;
  }
...
TABLE_LIST *copy_table(THD *thd, TABLE_LIST *src, Query_block *select,
                       Query_block *orig) {
...
bool copy_table_field(TABLE_LIST *src, TABLE_LIST *des, THD *thd,
...
bool copy_merge_table_field(THD *thd, Query_block *dest_select, int tableindex,
                            int mergeindex, TABLE_LIST *srctb) {
  TABLE_LIST *tb = get_table_by_index(dest_select->table_list.first,
                                      TABLE_LIST_TYPE_DEFAULT, tableindex);
  if (tb == nullptr) {
    return true;
  }
  TABLE_LIST *mergetable =
...
bool copy_global_table_list_field(THD *thd, Query_block *orig,
  ...
  for (TABLE_LIST *tbl_list = orig->leaf_tables; tbl_list != nullptr;
    ...
    if (tbl_list->field_translation != nullptr) {
      TABLE_LIST *src = get_table_by_index(dest_select->leaf_tables,
...
bool init_field_space(THD *thd, Query_block *orig, Query_block *select) {
  ...
  for (TABLE_LIST *tbl_list = orig->table_list.first; tbl_list != nullptr;
    ...
    if (tbl_list->merge_underlying_list != nullptr) {
      int mergeindex = 0;
      for (TABLE_LIST *tb = tbl_list->merge_underlying_list; tb != nullptr;
           tb = tb->merge_underlying_list) {
        if (tb->field_translation != nullptr) {
          TABLE_LIST *ta = get_table_by_index(
              select->table_list.first, TABLE_LIST_TYPE_DEFAULT, tableindex);
          if (ta == nullptr) {
            return true;
          }
          TABLE_LIST *mergetable =
...
bool copy_table_list_field(THD *thd, Query_block *orig,
  ...
  for (TABLE_LIST *tbl_list = orig->table_list.first; tbl_list != nullptr;
       tbl_list = tbl_list->next_local) {
    if (tbl_list->field_translation != nullptr) {
      TABLE_LIST *src = get_table_by_index(dest_select->table_list.first,
                                           TABLE_LIST_TYPE_DEFAULT, tableindex);
      if (src == nullptr) {
        return true;
      }
