...
TABLE_LIST *get_table_in_merge_tablelist(Query_block *select, TABLE_LIST *tb) {
  int tableindex = 0;
  for (TABLE_LIST *tbl_list = select->orig->table_list.first;
    ...
    if (tbl_list->merge_underlying_list != nullptr) {
      ...
      if (index != -1) {
        TABLE_LIST *tbl = get_table_by_index(
...
Item *Item_view_ref::pq_clone(class THD *thd, class Query_block *select) {
  ...
  if (select->orig != nullptr) {
    ...
    int index = get_table_index(select->orig->table_list.first,
                                TABLE_LIST_TYPE_DEFAULT, cached_table);
    if (index != -1) {
      found_table = get_table_by_index(select->table_list.first,
    ...
    *item_ref = (*ref)->pq_clone(thd, select);
    ...
    item = new (thd->pq_mem_root)
        Item_view_ref(&select->context, item_ref, db_name, table_name,
                      orig_table_name(), field_name, cached_table);
  }
