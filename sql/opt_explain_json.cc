...
bool table_base_ctx::format_body(Opt_trace_context *json,
  ...
  return format_gather(json) || format_derived(json) || format_query_expression(json);
}
...
class message_ctx : public joinable_ctx, public table_with_where_and_derived {
 public:
  ...
  virtual int add_gather(join_ctx *ctx, Query_expression *) {
...
class join_tab_ctx : public joinable_ctx, public table_with_where_and_derived {
 ...
 public:
  ...
  virtual int add_gather(join_ctx *ctx, Query_expression *subquery MY_ATTRIBUTE((unused))) {
    return gather.push_back(ctx);
  }
...
class simple_sort_ctx : public joinable_ctx {
 ...
 public:
  ...
  virtual int add_gather(join_ctx *ctx, Query_expression *subquery) {
...
int join_ctx::add_gather(join_ctx *ctx, Query_expression *subquery) {
  if (sort)
...
bool Explain_format_JSON::begin_context(enum_parsing_context ctx_arg,
  ...
  switch (ctx_arg) {
    case CTX_JOIN:
      ...
      break;
    case CTX_ORDER_BY: {
      assert(current_context->type == CTX_JOIN || current_context->type == CTX_GATHER);
