...
Field *Item_sum::create_tmp_field(bool, TABLE *table, MEM_ROOT *root) {
  ...
  switch (result_type()) {
    case REAL_RESULT:
      ...
      break;
    case INT_RESULT:
      field = new (pq_check_root) Field_longlong(max_length, is_nullable(),
  ...
  */
  switch (args[0]->data_type()) {
    case MYSQL_TYPE_DATE:
      field = new (pq_check_root) Field_newdate(is_nullable(), item_name.ptr());
      break;
...
Field *Item_rollup_sum_switcher::create_tmp_field(bool group, TABLE *table, MEM_ROOT *root) {
  return master()->create_tmp_field(group, table, root);
}
